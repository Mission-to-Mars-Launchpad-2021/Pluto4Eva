...
volatile int stopbonk = 3;

int sensorPinLeft = A7;    // select the input pin for the LDR
int sensorPinMid = A6;
...
#define MAX_DISTANCE_DOWN 200 // Maximum distance we want to ping for (in centimeters). Maximum sensor distance is rated at 400-500
...
void loop() {
  ...
  delay(2000);
  Off(1000);
  SlowReverse(); //forwards and reverse demo
  delay(2000);
  Off(8000); //talking about ledges
  
  Ramp(); //goes up, doesn't fall off
  Off(5000); //talk about obstacles
  
  Bonk(); //doesn't hit obstacle
  Off(5000);
...
void Ramp() {
  ...
  while (sonar_down.ping_cm()<15){
  SlowForward();
  Serial.print("Zooming");
  rightcounter = 0;}
    while (rightcounter<=322){
      SlowReverse();
...
void Bonk(){
 while (stopbonk > 1 ){
  rightcounter = 0;
    if ((sonar.ping_cm() >= 5) or (sonar.ping_cm() == 0)){ 
    SlowForward();
    }else{
    Off(1000);
    while (rightcounter <= 50){
      SlowReverse();
      delay(50);
    }
    Off(1000);
